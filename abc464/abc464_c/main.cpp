...
int main() {
    ...
    vector<vector<pair<int64_t,int64_t>>> ev(n);
    REP(i, n){
        int64_t a, d, b; cin >> a >> d >> b;
        a--; b--;
        if(d >= 2){
            ev[a].push_back({1, +1});
            ev[a].push_back({d, -1});
        ...
        ev[b].push_back({d, +1});
        ev[b].push_back({m+1, -1});
    }

    vector<int64_t> ans(m+2, 0);
    REP(c, n){
        auto &e = ev[c];
        if(e.empty()) continue;
        sort(e.begin(), e.end());
        int64_t cnt = 0, start = -1;
        int64_t i = 0, sz = e.size();
        while(i < sz){
            int64_t day = e[i].first;
            int64_t prev = cnt;
            while(i < sz && e[i].first == day){ cnt += e[i].second; i++; }
            if(prev == 0 && cnt > 0) start = day;          
