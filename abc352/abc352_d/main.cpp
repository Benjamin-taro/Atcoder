...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        int p;
        cin >> p;
        --p; // 0-based indexingに変換
        q[p] = i; // qにはp番目の要素のインデックスが入る
    }
    ...
    int ans = *st.rbegin() - *st.begin(); // 最初の範囲での差を計算
    // k以降の要素について処理
    for (int i = k; i < n; i++) {
        st.erase(q[i - k]); // 古い要素を削除
        st.insert(q[i]);    // 新しい要素を追加
        ans = min(ans, *st.rbegin() - *st.begin()); // 最小の差を更新
    }
