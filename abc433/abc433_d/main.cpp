...
int main() {
    ...
    cin >> n >> m;
    vector<int64_t> a(n);
    int64_t max_digits = 1;
    REP(i, n) {
        cin >> a[i];
        int64_t digits = to_string(a[i]).size();
        max_digits = max(max_digits, digits);
    }
    vector<unordered_map<int64_t, int64_t>> buckets(max_digits + 1);
    for(int64_t num : a) {
        num = num % m;
        buckets[0][num]++;
        for(int64_t d = 1; d <= max_digits; ++d) {
            num *= 10;
            num = num % m;
            buckets[d][num % m]++;
        }
    ...
    int64_t ans = 0;
    for(int64_t num : a) {
        int64_t dig = to_string(num).size();
        int64_t need = (m - (num % m)) % m;
        ans += buckets[dig][need];
    }
    cout << ans << "\n";
