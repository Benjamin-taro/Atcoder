...
int main() {
    ...
    // Use dynamic programming with a map to track differences
    unordered_map<int, unordered_map<int, int>> dp;

    for (int j = 0; j < N; ++j) {
        for (int i = 0; i < j; ++i) {
            if (H[i] == H[j]) { // Only consider buildings of the same height
                int diff = j - i; // Calculate interval
                dp[j][diff] = dp[i][diff] + 1;
                max_buildings = max(max_buildings, dp[j][diff] + 1);
            }
        }
    }
