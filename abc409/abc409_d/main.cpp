...
int main() {
    ...
    for(int64_t i = 0; i < t; i++) {
        ...
        for (int64_t j = 1; j < n; j++) {
            if(rotate) break;
            if (!rotate && prev_char > s[j]) {
                rot_start = j - 1;
                check_pos = j;

                while (check_pos < n) {
                    if (s[rot_start] < s[check_pos]) break;
                    check_pos++;
                }
                rotate = true;
            }
        ...
        // cerr << "rot_start: " << rot_start << ", check_pos: " << check_pos << "\n";

        if(!rotate) cout << s << "\n";
        else {
            string ans = "";
            for(int64_t k = 0; k < n; k++) {
                if(rot_start <= k && k < check_pos){
                    if(k != check_pos-1) ans += s[k+1];
                    else ans += s[rot_start];
                }
                else {
                    ans += s[k];
                }
