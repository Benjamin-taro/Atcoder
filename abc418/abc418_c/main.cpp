...
auto solve(int N, int Q, const std::vector<int64_t> &A, const std::vector<int64_t> &B) {
    // TODO: edit here
}
...
int main() {
    ...
    while(q--){
        int64_t b;
        std::cin >> b;
        auto it = lower_bound(ALL(a), b-1);
        int64_t index = it - a.begin();
        cerr << "index: " << index << "\n";
        cerr << "prefix_sum[index]: " << prefix_sum[index] << "\n";
        int64_t ans = 0;
        if(b > a[n-1]){
            cout << -1 << "\n";
            continue;
        ...
        cerr << "b: " << b << "\n";
        ans += prefix_sum[index] + (n - index) * (b-1);
        ans++;
        std::cout << ans << "\n";
    }
    return 0;
