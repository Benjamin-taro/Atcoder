...
int main() {
    ...
    REP(i,n){
        if(i==0){
            ...
            gap[i] = abs(cost[i]-cost[i-1]);
        }
    }
    gap[n] = abs(0-cost[n-1]);
    ...
    REP(i,n){
        int64_t ans = 0;
        if(i == 0){
            ans = sum - gap[i] - gap[i+1] + abs(cost[i+1]-0);
        }
        else if(i == n -1){
            ans = sum - gap[i] - gap[i+1] + abs(0-cost[i-1]);
        }
        else{
            ans = sum - gap[i] - gap[i+1] + abs(cost[i+1]-cost[i-1]);
        }
        std::cout << ans << '\n';
    }
