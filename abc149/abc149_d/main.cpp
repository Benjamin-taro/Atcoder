...
int64_t solve(int64_t N, int64_t K, int64_t R, int64_t S, int64_t P, const string& T) {
    ...
    for (int64_t i = 0; i < N; ++i) {
        ...
        // K手前と同じ手を出さないようにする
        if (i >= K && chosen[i - K] == win_hand) {
            // K手前と同じ手を出せない場合は0点で他の手を出す
            chosen[i] = 'x'; // 適当な手を出す（得点にならない）
        } else {
            // 勝てる手を出して得点を加算
            chosen[i] = win_hand;
            score += point;
