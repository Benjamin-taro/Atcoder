...
int64_t solve(int N, vector<int64_t> &L) {
    sort(ALL(L));
    int64_t ans = 0;
    for (int i = 0; i < N - 2; ++i) {
        int k = i + 2; // 最大の辺を表すためのポインタを初期化
        for (int j = i + 1; j < N - 1; ++j) {
            // sticks[i] + sticks[j] > sticks[k] を満たす最大の k を探す
            while (k < N && L[i] + L[j] > L[k]) {
                ++k;
            }
            // sticks[i] と sticks[j] を辺とする三角形で、
            // 3番目の辺が範囲 (j, k) 内にあるものの数を数える
            ans += k - j - 1;
        }
    }
    return ans;
