...
int main() {
    ...
    int64_t N, M, K;
    std::cin >> N >> M;
    std::vector<pair<int64_t, int64_t>> AB;
    REP (i, M) {
        int64_t a, b;
        cin >> a >> b;
        AB.push_back({a, b});
    ...
    std::cin >> K;
    std::vector<pair<int64_t, int64_t>> CD;
    REP (i, K) {
        int64_t c, d;
        cin >> c >> d;
        CD.push_back({c, d});
    ...
    int64_t ans = 0;
    REP(s, k2){
        vector<int64_t> dish(N+1);
        REP(i, K){
            if((s>>i)& 1) dish[CD[i].second]++;
            else dish[CD[i].first]++;
        }
        int64_t now = 0;
        REP(j, M){
            if (dish[AB[j].first] == 0 || dish[AB[j].second] == 0) continue;
            now++;
        }
        ans = max(ans, now);
    }
