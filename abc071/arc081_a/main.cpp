...
int main() {
    ...
    // 値をペアとしてベクトルに変換し、逆順にソート
    std::vector<std::pair<int64_t, int64_t>> v(a.begin(), a.end());
    std::sort(v.rbegin(), v.rend());
    
    int64_t lon = 0, shor = 0;
    int64_t area = 0;

    for (auto [key, value] : v) {
        if (value >= 4 && lon == 0) {
            area = std::max(area, key * key);
        }
        if (value >= 2 && lon == 0) {
            lon = key;
        } else if (value >= 2 && lon != 0) {
            shor = key;
            area = std::max(area, lon * shor);
            break; // 最初の2組のペアが見つかったら終了
        }
    }
