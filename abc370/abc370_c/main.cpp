...
std::string replaceCharacterAtIndex(const std::string& S, const std::string& T, std::size_t i) {
    // S と T の範囲内か確認
...
int main() {
    ...
    for(int i = 0; i < S.size(); i++){
        if(S[i] - T[i] < 0){
            checker.push_back(1);//S[i]がT[i]より小さい+
        }else if (S[i] - T[i] > 0){
            checker.push_back(-1);//S[i]がT[i]より大きい-
        }else{
            checker.push_back(0);//S[i]がT[i]と同じ
        }
    }
    for(int i = 0; i < S.size(); i++){
        if(checker[i] == -1){
            ans.push_back(replaceCharacterAtIndex(S, T, i));
            S = replaceCharacterAtIndex(S, T, i);
            checker[i] = 0;
        }
    }
