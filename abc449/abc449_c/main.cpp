...
int main() {
    ...
    REP(i, 26){
        ...
        REP(j, n){
            prefix[j+1] = prefix[j] + (s[j]==target ? 1:0);
        }
        for(int64_t j = 1; j+l <= n; j++){
            if(s[j-1] == target){
                if(j+r <=n) ans += max((int64_t)0, prefix[j+r]-prefix[j+l-1]);
                else ans += max((int64_t)0, prefix[n]-prefix[j+l-1]);
            }
        }
    }
    cout << ans <<"\n";
