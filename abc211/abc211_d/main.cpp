...
int main() {
    ...
    vector<int64_t> distance(n, -1); // -1 は未訪問
    vector<int64_t> ways(n, 0); // 通り数
    queue<int64_t> q;
    ...
    while (!q.empty()) {
        int64_t v = q.front();
        q.pop();
        for (int64_t u : path[v]) {
            if (distance[u] == -1) { // 未訪問なら
                distance[u] = distance[v] + 1;
                ways[u] = ways[v];
                q.push(u);
            } else if (distance[u] == distance[v] + 1) { // 最短距離で到達
