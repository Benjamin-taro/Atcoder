...
int main() {
    ...
    while(q--){
        ...
        if(t==1){
            ...
            queue<int> tsutsu;
            tsutsu.push(v);
            if(blackable.find(v) != blackable.end()) continue;
            while(!tsutsu.empty()){
                int now = tsutsu.front();
                tsutsu.pop();
                blackable.insert(now);
                for(auto to:g[now]){
                    if(blackable.find(to) != blackable.end()) continue;
                    tsutsu.push(to);
                }
            }
        }
        if(t==2){
            int v; cin >> v;
            if(blackable.find(v) != blackable.end()){
                cout << YES << "\n";
            }
            else{
                cout << NO << "\n";
