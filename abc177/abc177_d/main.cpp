...
void dfs(int node, std::vector<std::vector<int>>& adj, std::vector<bool>& visited, int64_t& current_group_size) {
    visited[node] = true;
    current_group_size++;

    for (int neighbor : adj[node]) {
        if (!visited[neighbor]) {
            dfs(neighbor, adj, visited, current_group_size);
        }
    }
}
...
int main() {
    ...
    for (int i = 0; i < N; ++i) {
        if (!visited[i]) {
            ...
            dfs(i, adj, visited, current_group_size);
            max_group_size = std::max(max_group_size, current_group_size);
        }
    }

    std::cout << max_group_size << std::endl;
    return 0;
}