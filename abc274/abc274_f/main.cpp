...
int64_t solve(int N, int64_t A, const std::vector<int64_t> &W, const std::vector<int64_t> &X, const std::vector<int64_t> &V) {
    // TODO: edit here
}
...
int main() {
    ...
    int64_t A;
    std::cin >> N;
    std::vector<int64_t> W(N), X(N), V(N);
    std::cin >> A;
    REP (i, N) {
        std::cin >> W[i] >> X[i] >> V[i];
    }
    auto ans = solve(N, A, W, X, V);
    std::cout << ans << '\n';
    return 0;
}
