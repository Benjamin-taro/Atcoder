...
int main() {
    ...
    vector<int64_t> dpo(10*n*2+1, 0);
    vector<int64_t> dpe(10*n*2+1, 0);
    dpe[10*n] = 1;
    REP(i, n){
        if(i == 0){
            dpo[10*n+a[0]] = 1;
        ...
        if(i%2 == 0){//x odd
            vector<int64_t> c_dpo(10*n*2+1, 0);
            REP(j, 10*n*2+1){
                if(dpo[j] == 1){
                    c_dpo[j+a[i]] = 1;
                    c_dpo[j-a[i]] = 1;
            ...
            vector<int64_t> c_dpe(10*n*2+1, 0);
            REP(j, 10*n*2+1){
                if(dpe[j] == 1){
                    c_dpe[j+a[i]] = 1;
                    c_dpe[j-a[i]] = 1;
    ...
    cout << (dpo[x+10*n] && dpe[y+10*n] ? YES : NO) << "\n";
    return 0;
}
