...
// Comparator to sort cards by strength descending and by cost ascending
bool comparator(const Card &a, const Card &b) {
...
int main() {
    ...
    // Sort cards by the custom comparator
    sort(cards.begin(), cards.end(), comparator);

    // Vector to hold the indices of the remaining cards
    vector<int> remaining_indices;
    int min_cost_so_far = INT_MAX;

    // Process cards to find which ones remain
    for (const auto &card : cards) {
        if (card.cost < min_cost_so_far) {
            remaining_indices.push_back(card.index);
            min_cost_so_far = card.cost;
        }
    }
    ...
    // Output the results
    cout << remaining_indices.size() << endl;
