...
auto solve(int64_t N, int64_t M, int Q, const std::vector<int64_t> &A, const std::vector<int64_t> &B, const std::vector<int64_t> &L, const std::vector<int64_t> &R) {
    // TODO: edit here
}
...
int main() {
    ...
    int Q;
    std::cin >> N >> M >> Q;
    std::vector<int64_t> A(Q), B(Q), L(Q), R(Q);
    REP (i, Q) {
        std::cin >> A[i] >> B[i] >> L[i] >> R[i];
    }
    auto ans = solve(N, M, Q, A, B, L, R);
