...
int main() {
    ...
    sort(ALL(c));
    reverse(ALL(c));
    vector<pair<int64_t, int64_t>> dp((1 << n), {1001001001, -1}); //{current track, and remaining capacity}
    dp[0] = {0, c[0]};
    for(int i = 0; i < (1<<n); i++){
        if(dp[i].second < 0) continue;
        int64_t k = dp[i].first;
        int64_t rem = dp[i].second;
        for(int j = 0; j < n; j++){
            if(!((i >> j) & 1)){
                int64_t nxt = i | (1<<j);
                int64_t nk = k, nrem= rem;
                if(w[j] <= rem){
                    nk = k;
                    nrem = rem-w[j];
                }
                else if(k+1 < m && w[j] <= c[k+1]){
                    nk = k+1; nrem = c[k+1]-w[j];
                }
                else{
                    continue;
                }
                auto npair = make_pair(nk, nrem);
                if (npair.first < dp[nxt].first || (npair.first == dp[nxt].first && npair.second > dp[nxt].second)) {
                    dp[nxt] = npair;
                }
            }
        }
