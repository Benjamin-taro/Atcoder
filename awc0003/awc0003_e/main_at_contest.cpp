...
int main() {
    ...
    //ぴったりのものだけ消す
    {
        REP(i, n){
            int64_t target = w[i];
            auto it = lower_bound(ALL(cc), target);
            if(*it == target && ccc[target] != 0){
                ccc[target]--;
                www[i] = false;
            }
        }
    ...
    REP(i, n){
        if(!www[i]) continue;
        int64_t largest = c.top();
        c.pop();
        if(largest < w[i]){
            cout << "No" << "\n";
            return 0;
        }
        largest = max((int64_t)0, largest-w[i]);
        if(largest) c.push(largest);
    }
    cout << "Yes" << "\n";
    return 0;
}