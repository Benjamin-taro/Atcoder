...
if [ $# -lt 1 ]; then
  echo "Usage: $0 <contest_number e.g. 0003 or 3>"
  exit 1
fi
...
CPP
  ...
  # oj t 用
  : > "${dir}/test/sample-1.in"
  : > "${dir}/test/sample-1.out"

  # run.sh
  cat > "${dir}/run.sh" <<'RUN'
#!/usr/bin/env bash
set -euo pipefail
g++-15 -std=c++17 main.cpp && oj t
RUN
  chmod +x "${dir}/run.sh"
done

echo "Done: created ${root}/ with tasks a..e"
