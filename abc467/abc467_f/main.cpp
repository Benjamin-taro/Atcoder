...
auto solve(int64_t a, int64_t b, int c, const std::vector<int64_t> &d, const std::vector<int64_t> &e, const std::vector<int64_t> &f, int64_t g, int64_t h) {
    // TODO: edit here
}
...
int main() {
    ...
    int64_t g, h;
    std::cin >> a >> b >> c;
    std::vector<int64_t> d(c), e(c), f(c);
    REP (i, c) {
        std::cin >> d[i] >> e[i] >> f[i];
    }
    std::cin >> g >> h;
