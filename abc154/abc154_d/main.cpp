...
double expected_value(double n) {
    double sum = 0;
    sum = ((1+n)/2) * n;
    return (double) sum/n;
}

double solve(int N, int64_t K, const std::vector<double> &p) {
    double ans = 0;
    double sum = 0;
    REP(i, N-K+1) {
        if(i == 0) {
            REP(j, K) {
                sum += expected_value(p[i+j]);
            }
        }
        else{
            sum = sum - expected_value(p[i-1]) + expected_value(p[i+K-1]);
        }
        cerr << sum << endl;
        ans = max(ans, sum);
    }
