...
int64_t dfs(int64_t used, int64_t n, string s, vector<int64_t> memo){
    if(memo[used] != -1) return memo[used];

    if(used == (1LL << n) - 1) return memo[used] = 1;

    for(int64_t i = 0; i < n; i++){
        if(used & (1LL << i)) continue;
        int64_t nxt = used | (1LL << i);
        if(s[nxt-1] == '0'){
            if(dfs(nxt, n, s, memo)){
                return memo[used] = 1;
            }
...
int main() {
    ...
    while (t--) {
        ...
        int64_t chemi = (1LL << n) - 1;
        vector<int64_t> memo(chemi + 1, -1);
