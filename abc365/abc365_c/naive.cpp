...
// ある価格 x で購入可能な総費用が M を超えないか確認する関数
bool is_feasible(const std::vector<int64_t>& A, int64_t x, int64_t M) {
    int64_t total_cost = 0;
    for (auto price : A) {
        total_cost += std::min(price, x);
        if (total_cost > M) return false;
    }
    return total_cost <= M;
}
...
int main() {
    ...
    int64_t accummulated = 0;
    int64_t ans = 0;
    int64_t expense = 0;

    // 条件に基づいて結果を計算
    for (int i = 0; i < N; ++i) {
        int64_t expense = prefix_sum[i] + A[i] * (N - i);
        if (expense <= M){
            ans = A[i];
        } 
        else{
