...
bool solve(int N, const std::vector<std::string> &s, const std::vector<std::string> &t) {
    // TODO: edit here
}
...
int main() {
    ...
    for (int i = 0; i < N; ++i) {
        ...
        name_count[names[i].first]++;
        name_count[names[i].second]++;
    }
    
    for (auto &name_pair : names) {
        if ((name_count[name_pair.first] > 1 && name_count[name_pair.second] > 1) ||
            (name_pair.first == name_pair.second && name_count[name_pair.first] > 1)) {
            cout << "No" << endl;
            return 0;
        }
    }
