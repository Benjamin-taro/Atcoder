...
int main() {
    ...
    int64_t n, m;
    cin >> n >> m;
    vector<vector<string>> maze(n);
    REP(i, n){
        maze[i].resize(m);
        string s; cin >> s;
        REP(j, m) maze[i][j] = s[j];
    }
    queue<pair<pair<int64_t, int64_t>, int64_t>> q; //point and hammer
    q.push({{0,0}, 0});
    int64_t INF = 1LL<<60;
    vector<pair<int64_t, int64_t>> directions = {{1,0}, {-1,0}, {0,1}, {0,-1}};
    vector<vector<int64_t>> visited(n, vector<int64_t>(m, INF));
    visited[0][0] = 0;
    while(!q.empty()){
        ...
        for(int64_t i = 0; i < 4; i++){
            ...
            int64_t cost = (maze[nh][nw] == "#" ? 1 : 0);
            if(visited[nh][nw] > visited[h][w]+cost){
                visited[nh][nw] = visited[h][w]+cost;
                q.push({{nh, nw}, cnt+cost});
            }
        }
    }
