...
// OpMax の id を安全な -INF にしておくのがおすすめ
template<class T> struct OpMax {
    T operator()(T a, T b) const { return std::max(a, b); }
    static T id() { return -(1LL<<60); } // ★ここを推奨
};
...
int main() {
    ...
    // dp[i] = max score when arriving at i
    vector<long long> dp(n, -(1LL<<60));

    SegTree<long long, OpMax<long long>> seg(n);
    ...
    for (int i = 1; i < n; i++) {
        int l = max(0, i - k);
        long long best = seg.query(l, i); // [l, i)
        dp[i] = best + a[i];
        seg.update(i, dp[i]);
    }
