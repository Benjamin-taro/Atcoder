...
int main() {
    ...
    vector<int64_t> prefix_prefix_sum(N + 1, 0);
    int64_t ans = 0;
    REP (i, N) {
        std::cin >> A[i];
        prefix_sum[i + 1] = prefix_sum[i] + A[i];
        prefix_prefix_sum[i + 1] = prefix_prefix_sum[i] + prefix_sum[i + 1];
    }
    int64_t temp = prefix_sum[0];
    REP(i, N+1){
        cerr << prefix_sum[i] << ' ';
        if(i == 0) continue;
        temp = max(temp, prefix_sum[i]);
        ans = max(ans, prefix_prefix_sum[i-1]+temp);
