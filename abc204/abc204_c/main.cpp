...
//bfs
int bfs(map<int, vector<int>>& mp, int start, int N) {
    ...
    queue<int> que;
    que.push(start);
    vector<int> visited(N + 1, -1); // 訪問状態を初期化（-1は未訪問を意味します）
    visited[start] = 0;
    int reachable = 0;
    ...
    while (!que.empty()) {
        ...
        que.pop();
        for (int u : mp[v]) {
            if (visited[u] == -1) { // 未訪問の都市のみを対象とします
