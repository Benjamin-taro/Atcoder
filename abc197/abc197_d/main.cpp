...
int main() {
    ...
    // 中心座標の計算
    double center_x = (ax + bx) / 2.0;
    double center_y = (ay + by) / 2.0;
    ...
    // 回転行列の適用
    double ansx = ax * cos(radian) - ay * sin(radian);
    double ansy = ax * sin(radian) + ay * cos(radian);
    ...
    // 結果の出力
    std::cout << fixed << setprecision(15) << ansx << ' ' << ansy << '\n';
