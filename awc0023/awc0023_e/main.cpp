...
int main() {
    ...
    REP(i, n) cin >> w[i];
    int64_t INF = 1LL<<60;
    vector<pair<int64_t, int64_t>> dp((1<<n), {INF, -1});
    dp[0] = {0, c};
    for(int64_t i = 0; i < (1<<n); i++){
        if(dp[i].second == -1) continue; // impossible
        int64_t num_track = dp[i].first;
        int64_t cap = dp[i].second;
        for(int64_t j=0; j<n; j++){
            if(!(i >> j &1)){
                int64_t nxt = i | (1 << j);
                int64_t nxt_track = num_track;
                int64_t nxt_rem = cap;
                if(w[j]<=cap){
                    nxt_rem -= w[j];
                }
                ...
                if((nxt_track < dp[nxt].first) || ((nxt_track == dp[nxt].first) && nxt_rem >= dp[nxt].second)){
                    dp[nxt] = {nxt_track, nxt_rem};
                }
            }
        }
    }
