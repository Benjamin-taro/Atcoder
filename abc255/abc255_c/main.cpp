...
int main() {
    ...
    std::cin >> X >> A >> D >> N;
    int64_t ans = 0;
    int64_t range_min = A, range_max = A + D * (N - 1);
    ...
    if (X < range_min) {
        ans = std::abs(range_min - X);
    } else if (X > range_max) {
        ans = std::abs(X - range_max);
    } else if (D == 0) {
        ans = std::abs(X - A);
    } else {
        // X が range_min と range_max の間にある場合
        int64_t k = (X - A) / D;
        if ((X - A) % D != 0 && X < A) k--;  // 修正点: 符号に依存する修正
        int64_t nearest = A + k * D;
        int64_t next = nearest + D;
        ans = std::min(std::abs(nearest - X), std::abs(next - X));
    }
