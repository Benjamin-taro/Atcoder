...
int main() {
    ...
    for(auto [to, cost]:edges[0]){
        auto dfs = [&](auto&& self, int64_t cur, int64_t cost_sum, int64_t depth) -> void {
            if(cost_sum > t){
                return;
            }
            if(depth == l){
                if(s <= cost_sum && cost_sum <= t){
                    result[cur] = true;
                }
                return;
            }
            for(auto [nxt, nc]:edges[cur]){
                self(self, nxt, cost_sum+nc, depth + 1);
            }
        };
        dfs(dfs, 0, 0, 0);
    }
