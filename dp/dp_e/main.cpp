...
int main() {
    ...
    // DPテーブルの作成: dp[i][j] = i番目までのアイテムで、価値の総和がjのときの重さの最小値
    // 初期値は無限大(ここでは W+1 で代用)で、dp[0][0] = 0
    std::vector<std::vector<int64_t>> dp(N+1, std::vector<int64_t>(100001, W+1));
    dp[0][0] = 0;

    for (int i = 0; i < N; ++i) {
        for (int j = 0; j <= 100000; ++j) {
            if (j - v[i] >= 0) {
                // アイテムiを追加する場合、重さの総和が増加
    ...
    // 結果を探す: 価値の総和が最大で、重さの合計がW以下のものを探す
    int64_t res = 0;
    for (int j = 0; j <= 100000; ++j) {
        if (dp[N][j] <= W) {
            res = j;
