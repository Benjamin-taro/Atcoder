...
int main() {
    ...
    reverse(ALL(a));
    vector<int64_t> prefix_sum_rev(N + 1, 0);
    REP(i, N) {
        prefix_sum_rev[i + 1] = prefix_sum_rev[i] + a[i];
    }
    REP(i, N + 1) {
        cerr << prefix_sum[i] << " ";
        cerr << prefix_sum_rev[i] << endl;
    }
    int64_t ans = 1e18;
    for(int i = 1; i < N; i++) {
        cerr << abs(prefix_sum[i]-prefix_sum_rev[N - i]) << endl;
        ans = min(ans, abs(prefix_sum[i]-prefix_sum_rev[N - i]));
    }
    std::cout << ans << '\n';
    return 0;
}
