...
int main() {
    ...
    REP(i, n) {
        sum_r += ranges[i].second;
        sum_l += ranges[i].first;
    }
    ...
    int64_t current_gap = 0-sum_l;
    cout << "Yes" << endl;
    REP(i, n){
        if(current_gap >= 0){
            cout << ranges[i].first + min(current_gap, abs(ranges[i].second-ranges[i].first)) << " ";
        }
        current_gap -= min(current_gap, abs(ranges[i].second-ranges[i].first));
    }
