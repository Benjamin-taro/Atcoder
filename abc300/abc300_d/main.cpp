...
int main() {
    ...
    int64_t n; cin >> n;
    set<int64_t> ans;
    for(int64_t i = 1; i*i*i*i*i <= n; i++){
        if(!IsPrime(i)) continue;
        for(int64_t j = i+1; i*i*j*j*j <= n; j++){
            if(!IsPrime(j)) continue;
            for(int64_t k = j+1; i*i*j*k*k<=n; k++){
                if(!IsPrime(k)) continue;
                if(i*i*j*k*k < 300 || i*i*j*k*k > 1000000000000) continue;
                ans.insert(i*i*j*k*k);
            }
        }
    }
    cout << ans.size() << "\n";
