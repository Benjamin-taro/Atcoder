...
bool solve(auto H, auto W, const std::vector<std::vector<auto> > &A, const std::vector<std::vector<auto> > &B) {
    // TODO: edit here
}
...
int main() {
    ...
    auto H, W;
    std::cin >> H >> W;
    std::vector<std::vector<auto> > A(H + W + 4, std::vector<auto>((H + W + 4))), B(H + W + 4, std::vector<auto>((H + W + 4)));
    REP (j, H + 4) {
        REP (i, W) {
            std::cin >> A[i + j][i + j];
        }
