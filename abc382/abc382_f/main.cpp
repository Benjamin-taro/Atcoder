...
auto solve(int64_t H, int64_t W, int N, const std::vector<int64_t> &R, const std::vector<int64_t> &C, const std::vector<int64_t> &L) {
    // TODO: edit here
}
...
int main() {
    ...
    int N;
    std::cin >> H >> W >> N;
    std::vector<int64_t> R(N), C(N), L(N);
    REP (i, N) {
        std::cin >> R[i] >> C[i] >> L[i];
    }
    auto ans = solve(H, W, N, R, C, L);
    REP (i, N) {
        std::cout << ans[i] << '\n';
    }
    return 0;
