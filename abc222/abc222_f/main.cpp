...
auto solve(int N, const std::vector<int64_t> &A, const std::vector<int64_t> &B, const std::vector<int64_t> &C, const std::vector<int64_t> &D) {
    // TODO: edit here
}
...
int main() {
    ...
    int N;
    std::cin >> N;
    std::vector<int64_t> A(N - 1), B(N - 1), C(N - 1), D(N);
    REP (i, N - 1) {
        std::cin >> A[i] >> B[i] >> C[i];
    }
    REP (i, N) {
