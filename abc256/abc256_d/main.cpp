...
auto solve(int N, const std::vector<int64_t> &L, const std::vector<int64_t> &R) {
    // TODO: edit here
}
...
int main() {
    ...
    int64_t N;
    std::cin >> N;
    std::vector<pair<int64_t, int64_t>> D;
    REP (i, N) {
        int64_t L, R;
        std::cin >> L >> R;
        D.emplace_back(L, R);
    ...
    int64_t temp_l = 0, temp_r = 0;
    REP(i, N){
        if(i == 0){
            temp_l = D[i].first;
            temp_r = D[i].second;
            if(i == N-1) cout << temp_l << " " << temp_r << endl;
        }
        else{
            if(D[i].first <= temp_r){
                temp_r = max(temp_r, D[i].second);
                if(i == N-1) cout << temp_l << " " << temp_r << endl;
            }
            else{
