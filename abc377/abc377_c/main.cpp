...
int main() {
    ...
    int M;
    std::cin >> N >> M;
    set<pair<int64_t, int64_t>> possible_positions;
    REP (i, M) {
        int64_t a, b;
        std::cin >> a >> b;
        possible_positions.emplace(a, b);
        if(a+2<=N && a+2 >=1 && b+1<=N && b+1>=1) possible_positions.emplace(a+2, b+1);
        if(a+2<=N && a+2 >=1 && b-1<=N && b-1>=1) possible_positions.emplace(a+2, b-1);
        if(a-2<=N && a-2 >=1 && b+1<=N && b+1>=1) possible_positions.emplace(a-2, b+1);
        if(a-2<=N && a-2 >=1 && b-1<=N && b-1>=1) possible_positions.emplace(a-2, b-1);
        if(a+1<=N && a+1 >=1 && b+2<=N && b+2>=1) possible_positions.emplace(a+1, b+2);
        if(a+1<=N && a+1 >=1 && b-2<=N && b-2>=1) possible_positions.emplace(a+1, b-2);
        if(a-1<=N && a-1 >=1 && b+2<=N && b+2>=1) possible_positions.emplace(a-1, b+2);
        if(a-1<=N && a-1 >=1 && b-2<=N && b-2>=1) possible_positions.emplace(a-1, b-2);
    }
    int64_t ans = 0;
    ans = N*N - possible_positions.size();
    std::cout << ans << '\n';
    return 0;
}
