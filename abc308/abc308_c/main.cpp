...
int main() {
    ...
    int64_t N;
    std::cin >> N;
    std::vector<std::pair<int64_t, std::pair<int64_t, int64_t>>> A;  // インデックスと(a, b)のペアを保持
    REP (i, N) {
        int64_t a, b;
        std::cin >> a >> b;
        A.emplace_back(i + 1, std::make_pair(a, b));
    }

    std::sort(ALL(A), [](const auto &lhs, const auto &rhs) {
    if (lhs.second.first * (rhs.second.first + rhs.second.second) != rhs.second.first * (lhs.second.first + lhs.second.second)) {
        return lhs.second.first * (rhs.second.first + rhs.second.second) > rhs.second.first * (lhs.second.first + lhs.second.second);
    } else {
        return lhs.first < rhs.first;  // 同じ確率の場合、インデックスの昇順でソート
    }
});
