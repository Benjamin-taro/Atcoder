...
int64_t solve(int N, const std::vector<int64_t> &r, const std::vector<int64_t> &c, const std::vector<int64_t> &x) {
    // TODO: edit here
}
...
int main() {
    ...
    int N;
    std::cin >> N;
    std::vector<int64_t> r(N), c(N), x(N);
    REP (i, N) {
        std::cin >> r[i] >> c[i] >> x[i];
    }
    auto ans = solve(N, r, c, x);
    std::cout << ans << '\n';
    return 0;
}
