...
auto solve(int N, const std::vector<int64_t> &U, const std::vector<int64_t> &V, const std::vector<int64_t> &L) {
    // TODO: edit here
}
...
int main() {
    ...
    int N;
    std::cin >> N;
    std::vector<int64_t> U(N - 1), V(N - 1), L(N - 1);
    REP (i, N - 1) {
        std::cin >> U[i] >> V[i] >> L[i];
    }
    auto ans = solve(N, U, V, L);
    REP (i, N) {
        std::cout << ans[i] << '\n';
    }
    return 0;
