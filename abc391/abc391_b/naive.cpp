...
std::pair<int64_t, int64_t> solve(auto N, auto M, const std::vector<std::vector<auto> > &S, const std::vector<std::vector<auto> > &T) {
    // TODO: edit here
}
...
int main() {
    ...
    auto N, M;
    std::cin >> N;
    std::vector<std::vector<auto> > S(2 * N + 4, std::vector<auto>((2 * N + 4)));
    std::cin >> M;
    std::vector<std::vector<auto> > T(2 * M + 4, std::vector<auto>((2 * M + 4)));
    REP (j, N + 4) {
        REP (i, N) {
            std::cin >> S[i + j][i + j];
        }
