...
vector<int> bfs(int v, map<int, vector<int>>& g, int Y) {
    queue<int> q;
    unordered_map<int, int> parent;
    q.push(v);
    parent[v] = -1;  // Start node has no parent

    while (!q.empty()) {
        int u = q.front();
        q.pop();
        for (auto w : g[u]) {  // Ensure u is an int
            if (parent.find(w) == parent.end()) {  // Check if w is not visited
                q.push(w);
                parent[w] = u;
                if (w == Y) {
                    vector<int> path;
