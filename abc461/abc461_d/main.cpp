...
int main() {
    ...
    // 2次元prefix sum
    vector<vector<int64_t>> prefix(h+1, vector<int64_t>(w+1, 0));
    REP(i, h) {
        REP(j, w) {
            prefix[i+1][j+1] = prefix[i][j+1] + prefix[i+1][j] - prefix[i][j] + (s[i][j] - '0');
        }
    }
    
    int64_t ans = 0;
    
    vector<int64_t> cnt(h * w + 1, 0);
    vector<int64_t> used;
    used.reserve(w + 1);
    
    for (int64_t top = 0; top < h; ++top) {
        for (int64_t bottom = top; bottom < h; ++bottom) {
            
            // クリア
            for (int64_t idx : used) cnt[idx] = 0;
            used.clear();
            ...
            for (int64_t j = 0; j < w; ++j) {
                cur += prefix[bottom+1][j+1] - prefix[top][j+1]
                     - prefix[bottom+1][j]   + prefix[top][j];
                
                if (cur - k >= 0) ans += cnt[cur - k];
                if (cnt[cur] == 0) used.push_back(cur);
                cnt[cur]++;
            }
        }
    }
