...
int64_t solve(int N, int64_t X, const std::vector<int64_t> &V, const std::vector<int64_t> &A, const std::vector<int64_t> &C) {
    // TODO: edit here
}
...
int main() {
    ...
    int64_t X;
    std::cin >> N;
    std::vector<int64_t> V(N), A(N), C(N);
    std::cin >> X;
    REP (i, N) {
        std::cin >> V[i] >> A[i] >> C[i];
    }
    auto ans = solve(N, X, V, A, C);
    std::cout << ans << '\n';
    return 0;
}
