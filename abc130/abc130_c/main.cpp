...
std::pair<double, int64_t> solve(int64_t W, int64_t H, int64_t x, int64_t y) {
    double max_area = W * H / 2.0;
    int64_t ans = 0;
    if (x == W / 2.0 && y == H / 2.0) {
        ans = 1;
    }

    return std::make_pair(max_area, ans);
}
...
int main() {
    ...
    int64_t W, H, x, y;
    std::cin >> W >> H >> x >> y;
    auto [c, d] = solve(W, H, x, y);
    std::cout << std::fixed << std::setprecision(6) << c << ' ' << d << '\n';
    return 0;
}