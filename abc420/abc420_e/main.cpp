...
int main() {
    ...
    vector<bool> color(n, false); // falseが白、trueが黒
    while(q--){
        int t; cin >> t;
        if(t == 1){
            int u, v; cin >> u >> v;
            --u; --v;
            int lu = d.leader(u); int lv = d.leader(v);
            if(lu != lv){
                int new_sum = black[lu] + black[lv];
                d.merge(u, v);
                int new_leader = d.leader(lu);
                black[new_leader] = new_sum;
                black[lu] = new_sum; black[lv] = new_sum;
            }
        }
        else if(t == 2){
            int v; cin >> v;
            --v;
            int leader = d.leader(v);
            if(color[v]) {
                black[leader]--;
            } else {
                black[leader]++;
