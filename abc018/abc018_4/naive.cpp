...
int64_t solve(int64_t N, int64_t M, int64_t P, int64_t Q, int R, const std::vector<int64_t> &x, const std::vector<int64_t> &y, const std::vector<int64_t> &z) {
    // TODO: edit here
}
...
int main() {
    ...
    int R;
    std::cin >> N >> M >> P >> Q >> R;
    std::vector<int64_t> x(R), y(R), z(R);
    REP (i, R) {
        std::cin >> x[i] >> y[i] >> z[i];
    }
    auto ans = solve(N, M, P, Q, R, x, y, z);
    std::cout << ans << '\n';
    return 0;
}
