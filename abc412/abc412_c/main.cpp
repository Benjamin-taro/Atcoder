...
int main() {
    ...
    while(t--){
        ...
        sort(ALL(middle));
        int64_t ans = 2;
        int64_t target = start*2;
        cerr << "target start: " << target << ", goal: " << goal << "\n";
        while(target <= goal){
            cerr << "target: " << target << "\n";
            //二分探索でtarget以下の最大値を探す
            auto it = upper_bound(ALL(middle), target);
            if(it == middle.begin()){
                ans = -1;
                break; 
            }
