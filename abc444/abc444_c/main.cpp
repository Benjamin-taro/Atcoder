...
int main() {
    ...
    sort(ALL(a));
    set<int64_t> ans;
    if(a[0] == a[n-1]){ // all not broken
        ans.insert(a[0]);
    }
    { // larget is not broken
        int64_t not_broken = a[n-1];
        auto it = lower_bound(ALL(a), not_broken);
        int64_t idx = distance(a.begin(), it);
        if(idx%2 == 0){
            bool valid = true;
            for(int64_t i = 0; i < idx; i++){
                if(a[i]+a[idx - 1 - i] != not_broken){
                    valid = false;
                    break;
                }
            }
            if(valid){
                ans.insert(not_broken);
            }
        }
    }
    { //everything broken
        int64_t original_size = a[0] + a[n-1];
        if(n%2 == 0){
            bool valid = true;
            for(int64_t i = 0; i < n; i++){
                if(a[i]+a[n - 1 - i] != original_size){
                    valid = false;
                    break;
                }
            }
            if(valid){
                ans.insert(original_size);
            }
        }
    }
    for(auto v: ans){
