...
auto solve(int N, const std::vector<int64_t> &U, const std::vector<int64_t> &D, const std::vector<int64_t> &L, const std::vector<int64_t> &R) {
    // TODO: edit here
}
...
int main() {
    ...
    std::cin.tie(nullptr);
    int64_t n; cin >> n;
    vector<vector<int64_t>> sky (2001, vector<int64_t>(2001, 0));
    vector<vector<int64_t>> one (2001, vector<int64_t>(2001, 0));
    ...
    REP(i, 2000)REP(j, 2000) sky[i+1][j] += sky[i][j];
    REP(i, 2000)REP(j, 2000) sky[i][j+1] += sky[i][j];
    
    REP(i, 2000)REP(j, 2000) one[i+1][j+1] = (sky[i][j]==1);
    REP(i, 2000)REP(j, 2001) one[i+1][j] += one[i][j];
    REP(i, 2001)REP(j, 2000) one[i][j+1] += one[i][j];
    
    
    int64_t base = 0;
    REP(i, 2000)REP(j, 2000) if(sky[i][j] == 0) base++;

    REP(i, n){
        auto [u, d, l, r] = clouds[i];
        int64_t dif = one[d][r] - one[d][l] - one[u][r]+one[u][l];
        int64_t ans = base+dif;
        cout << ans << "\n";
    }
