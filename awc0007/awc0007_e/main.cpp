...
int main() {
    ...
    goal = {(t-1)/n, (t-1)%n};
    vector<pair<int64_t, int64_t>> route;
    route.push_back(start);
    REP(i, m){
        int64_t p;
        cin >> p;
        route.emplace_back((p-1)/n, (p-1)%n);
    }
    route.push_back(goal);
    int64_t INF = 1LL<<60;
    vector<vector<int64_t>>dp(1<<(m+2), vector<int64_t> (m+2, INF));
    dp[1<<0][0] = 0;
    auto dist = [&](int64_t a, int64_t b){
        return abs(route[a].first-route[b].first) + abs(route[a].second-route[b].second);
    };
    ...
    for(int i = 0; i < (1<<(m+2)); i++){
        for(int j = 0; j < m+2; j++){
            ...
            for(int64_t k=0; k < m+2; k++){
                if(i&(1<<k)) continue; // 
                int64_t newmask = i | (1<<k);
                dp[newmask][k] = min(dp[newmask][k], dp[i][j]+dist(j, k));
            }
        }
    }
    cout << dp[(1<<(m+2))-1][m+2-1] << "\n";
