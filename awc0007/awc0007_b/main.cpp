...
int main() {
    ...
    int64_t n, k;
    cin >> n >> k;
    vector<vector<string>> lab(n);
    REP(i, n){
        int64_t m; cin >> m;
        REP(j, m){
            string theme = "";
            cin >> theme;
            lab[i].push_back(theme);
        }
    }
    int64_t ans = 0;
    for(int64_t i = 0; i < n-1; i++){
        for(int64_t j = i+1; j < n; j++){
            set<string> aggre;
            for(auto t:lab[i]) aggre.insert(t);
            for(auto t:lab[j]) aggre.insert(t);
            if((int64_t)lab[i].size()+(int64_t)lab[j].size()-k >= (int64_t)aggre.size()) ans++;
        }
    }
    cout << ans << "\n";
