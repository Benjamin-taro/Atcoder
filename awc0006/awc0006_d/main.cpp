...
int main() {
    ...
    int64_t n, m;
    cin >> n >> m;
    vector<pair<int64_t, int64_t>> keibi;
    REP(i, m){
        int64_t l, r;
        cin >> l >> r;
        keibi.emplace_back(l, r);
    }
    sort(ALL(keibi), [](const auto& a, const auto& b){
        if(a.first != b.first)
    ...
    while(cur <= n){
        int64_t best = -INF;
        while(cnt < m && keibi[cnt].first <= cur){
            best = max(best, keibi[cnt].second);
            cnt++;
        }
        if(best < cur){
            cout << -1 << "\n";
