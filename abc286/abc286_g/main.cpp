...
bool solve(int64_t N, int M, const std::vector<int64_t> &U, const std::vector<int64_t> &V, int K, const std::vector<int64_t> &x) {
    // TODO: edit here
}
...
int main() {
    ...
    std::cin >> K;
    std::vector<int64_t> x(K);
    REP (i, K) {
        std::cin >> x[i];
    }
    auto ans = solve(N, M, U, V, K, x);
    std::cout << (ans ? YES : NO) << '\n';
    return 0;
}
