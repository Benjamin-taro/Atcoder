...
int main() {
    ...
    REP(i, n){
        vector<int64_t> visited(n, INF);
        visited[i] = 0;
        queue<int64_t> q;
        q.push(i);
        while(!q.empty()){
            int64_t a = q.front(); q.pop();
            if(a+1<n && visited[a+1] == INF){
                visited[a+1] = visited[a]+1;
                q.push(a+1);
            }
            if(a-1 >= 0 && visited[a-1]==INF) {
                visited[a-1] = visited[a]+1;
                q.push(a-1);
            }
            if(a==x && visited[y]==INF){
                visited[y] = visited[a]+1;
                q.push(y);
            }
            if(a==y && visited[x]==INF){
                visited[x] = visited[a]+1;
                q.push(x);
            }
        }
        REP(i, n){
            if(visited[i] != INF) ans[visited[i]]++;
        }
    }
    int64_t cnt = 0;
    REP(i, n){
