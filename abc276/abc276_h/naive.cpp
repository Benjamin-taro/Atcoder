...
auto solve(int64_t N, int Q, const std::vector<int64_t> &a, const std::vector<int64_t> &b, const std::vector<int64_t> &c, const std::vector<int64_t> &d, const std::vector<int64_t> &e) {
    // TODO: edit here
}
...
int main() {
    ...
    int Q;
    std::cin >> N >> Q;
    std::vector<int64_t> a(Q), b(Q), c(Q), d(Q), e(Q);
    REP (i, Q) {
        std::cin >> a[i] >> b[i] >> c[i] >> d[i] >> e[i];
    }
    auto ans = solve(N, Q, a, b, c, d, e);
    REP (j, N + 4) {
        REP (i, N) {
            std::cout << x[i + j][i + j] << ' ';
        }
