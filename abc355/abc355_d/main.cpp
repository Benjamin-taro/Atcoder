...
int main() {
    ...
    long long overlappingCount = 0;

    for (const auto &interval : intervals) {
        // Remove intervals that are no longer overlapping
        while (!activeEnds.empty() && *activeEnds.begin() < interval.start) {
            activeEnds.erase(activeEnds.begin());
        }

        // All remaining intervals in the multiset overlap with the current interval
        overlappingCount += activeEnds.size();

        // Add the current interval's end to the multiset
        activeEnds.insert(interval.end);
    }

    cout << overlappingCount << endl;
    return 0;
}
