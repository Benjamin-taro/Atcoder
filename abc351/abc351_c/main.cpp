...
stack<int64_t> solve(stack<int64_t> ans, int64_t a) {
    if(ans.empty()){
        ans.push(a);
        return ans;
    }
    else{
        int64_t temp = ans.top(); //一番末尾にいるやつ
        if(temp != a){
            ans.push(a);
            return ans;
        }
...
int main() {
    ...
    std::stack<int64_t> balls;
    for (int i = 0; i < N; i++) {
        int64_t Ai;
        std::cin >> Ai;

        // サイズが同じボールを組み合わせる
        while (!balls.empty() && balls.top() == Ai) {
            balls.pop();
            Ai += 1;
        }

        // 新しいボール（または組み合わせた後のボール）をプッシュ
        balls.push(Ai);
    }
