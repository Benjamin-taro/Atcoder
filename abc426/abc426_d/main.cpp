...
int main() {
    ...
    while(t--) {
        ...
        // 0と1の総数
        vector<int64_t> cnt(2, 0);
        REP(i, s.size()) cnt[s[i]-'0']++;
        
        // ランレングス圧縮
        vector<pair<char,int64_t>> comp;
        REP(i, s.size()) {
            if(i == 0) comp.push_back({s[i], 1});
            else if(s[i] == s[i-1]) comp.back().second++;
        ...
        for(auto [c, num] : comp) {
            int64_t i = c - '0';
            vector<int64_t> ncnt = cnt;
            ncnt[i] -= num;  // この区間を除く
            int64_t now = ncnt[i]*2 + ncnt[i^1];
            ans = min(ans, now);
        }
