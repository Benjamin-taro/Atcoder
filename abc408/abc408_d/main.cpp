...
int main() {
    ...
    while(t--){
        int64_t n; cin >> n;
        string s; cin >> s;
        vector<int64_t> prefix1(n+1, 0);
        vector<int64_t> prefix0(n+1, 0);
        REP(i, n){
            if(s[i] == '1'){
        ...
        dp[0][0] = 0;
        dp[0][1] = 1e18;  // まだ1ブロック開始してないのにj=1はありえない
        dp[0][2] = 1e18;  // 同様

        REP(i, n){
            int c = s[i] - '0';  // 現在の文字（0 or 1）
            
            // j=0: ずっと0ゾーン → 1が来たらコスト+1
            dp[i+1][0] = dp[i][0] + (c == 1 ? 1 : 0);
            
            // j=1: 1ゾーン → 0が来たらコスト+1
            dp[i+1][1] = min(dp[i][0], dp[i][1]) + (c == 0 ? 1 : 0);
            
            // j=2: 1ゾーン終了後の0ゾーン → 1が来たらコスト+1
            dp[i+1][2] = min(dp[i][1], dp[i][2]) + (c == 1 ? 1 : 0);
        }
