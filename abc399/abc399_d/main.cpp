...
int main() {
    ...
    while(t--){
        ...
        vector<vector<int64_t>> positions(n);
        REP(i, n) {
            positions[a[i]].push_back(i);
        }
        int64_t ans = 0;
        REP(i, n){
            int64_t next = a[positions[i][0]];
            if(abs(positions[i][0]-positions[i][1]) != 1 && abs(positions[next][0]-positions[next][1]) != 1){
                vector<int64_t> tmp;
                tmp.push_back(positions[i][0]);
                tmp.push_back(positions[i][1]);
                tmp.push_back(positions[next][0]);
                tmp.push_back(positions[next][1]);
                sort(tmp.begin(), tmp.end());
                if(tmp[1]-tmp[0] == 1 && tmp[2]-tmp[3] == 1){
                    ans++;
                }
            }
        }
