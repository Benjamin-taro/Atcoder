...
int main() {
    ...
    int64_t k; cin >> k;
    string s, t; cin >> s >> t;
    vector<int64_t> stren_s(10, 1);
    vector<int64_t> stren_t(10, 1);
    int64_t score_s = 0;
    int64_t score_t = 0;
    vector<int64_t> used(10, k);
    REP(i, 4){
        stren_s[s[i]-'0']*=10;
        used[s[i]-'0']--;
    }
    ...
    for(int i = 1; i <=9; i++){
        ...
        for(int j = 1; j <=9; j++){
            ...
            total += (used[i]+1)*used[j];
            auto f_s = stren_s;
            auto f_t = stren_t;
            f_s[i]*=10; f_t[j]*=10;
            int64_t fs_s = 0, fs_t = 0;
            for(int k = 1; k <=9; k++){
                fs_s += k*f_s[k];
                fs_t += k*f_t[k];
            }
            if(fs_s > fs_t) taka_win+=(used[i]+1)*used[j];
        }
        used[i]++;
    }
    long double ans = (long double)taka_win/total;
