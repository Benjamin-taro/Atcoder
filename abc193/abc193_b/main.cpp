...
int64_t solve(int N, const std::vector<int64_t> &A, const std::vector<int64_t> &P, const std::vector<int64_t> &X) {
    // TODO: edit here
}
...
int main() {
    ...
    int N;
    std::cin >> N;
    std::vector<int64_t> A(N), P(N), X(N);
    REP (i, N) {
        std::cin >> A[i] >> P[i] >> X[i];
    }
    auto ans = solve(N, A, P, X);
    std::cout << ans << '\n';
    return 0;
}
