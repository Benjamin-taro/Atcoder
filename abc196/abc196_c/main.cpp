...
int64_t solve(int64_t N) {
    int64_t cnt = 0;
    // 1桁の数からスタートし、Nの半分の桁数まで検討する（Nの桁数の半分を超えると条件を満たす数は存在しないため）
    for (int64_t len = 1; len <= std::to_string(N).length() / 2; ++len) {
        int64_t start = std::pow(10, len - 1);
        int64_t end = std::pow(10, len) - 1;
        for (int64_t i = start; i <= end; ++i) {
            // iを文字列に変換し、その反転を取得
            std::string firstHalf = std::to_string(i);
            std::string secondHalf(firstHalf.rbegin(), firstHalf.rend());
            // 前半と後半を結合して条件を満たす数を生成
            int64_t palindrome = std::stoll(firstHalf + firstHalf);
            // 生成した数がN以下であればカウント
            if (palindrome <= N) {
                ++cnt;
            } else {
                // 一度でもNを超えたら、それ以降の数は全てNを超えるため、ループを抜ける
                break;
            }
        }
    }
