...
int main() {
    ...
    vector<pair<int64_t, int64_t>> a(n1), b(n2);
    for (int64_t i = 0; i < n1; ++i) cin >> a[i].first >> a[i].second;
    for (int64_t i = 0; i < n2; ++i) cin >> b[i].first >> b[i].second;

    int64_t i = 0, j = 0;
    int64_t cnt = 0;

    while (i < n1 && j < n2) {
        int64_t len = min(a[i].second, b[j].second);
        if (a[i].first == b[j].first) cnt += len;
