...
int64_t solve(int64_t N, int M, const std::vector<int64_t> &u, const std::vector<int64_t> &v, const std::vector<int64_t> &l) {
    // TODO: edit here
}
...
int main() {
    ...
    int M;
    std::cin >> N >> M;
    std::vector<int64_t> u(M), v(M), l(M);
    REP (i, M) {
        std::cin >> u[i] >> v[i] >> l[i];
    }
    auto ans = solve(N, M, u, v, l);
    std::cout << ans << '\n';
    return 0;
}
