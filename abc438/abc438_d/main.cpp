...
int main() {
    ...
    vector<int64_t> pre_a(n + 1, 0), pre_b(n + 1, 0), pre_c(n + 1, 0);
    REP (i, n) {
        std::cin >> a[i];
    ...
    for(int64_t i = 1; i <= n; ++i) {
        int64_t snake = pre_a[i];// only head0-i
        int64_t x = i, y = n; // body x-y, tail y-n
        int64_t best = -INF;
        while(x <= y) {
            int64_t mid = (x + y) / 2;
            int64_t body = pre_b[mid] - pre_b[i - 1];
            int64_t tail = pre_c[n] - pre_c[mid - 1];
            if(body + tail > best) {
                best = body + tail;
            }
            if(body + tail < best) {
