...
int64_t solve(int N, int64_t X, const std::vector<int64_t> &A, const std::vector<int64_t> &P, const std::vector<int64_t> &B, const std::vector<int64_t> &Q) {
    // TODO: edit here
}
...
int main() {
    ...
    int64_t X;
    std::cin >> N;
    std::vector<int64_t> A(N), P(N), B(N), Q(N);
    std::cin >> X;
    REP (i, N) {
        std::cin >> A[i] >> P[i] >> B[i] >> Q[i];
    }
    auto ans = solve(N, X, A, P, B, Q);
    std::cout << ans << '\n';
    return 0;
}
