...
int main() {
    ...
    std::cin.tie(nullptr);
    int64_t n; cin >> n;
    vector<tuple<int64_t, int64_t, int64_t>> csf;
    REP(i, n-1){
        int64_t c, s, f; cin >> c >> s >> f;
        csf.emplace_back(c, s, f);
    }
    REP(i, n-1){
        int64_t cnt = 0;
        auto [cc, ss, ff] = csf[i];
        cnt+=cc+ss;
        cerr << "cnt: " << cnt << ", cc: " << cc << ", ss: " << ss << ", ff: " << ff << "\n";
        for(int j = i+1; j < n-1; j++){
            auto [nc, ns, nf] = csf[j];
            if(cnt-ns<0) cnt = ns+nc;
            else cnt = ((cnt+nf-1)/nf)*nf+nc;
            cerr << "cnt: " << cnt << ", nc: " << nc << ", ns: " << ns << ", nf: " << nf << "\n";
        }
        cout << cnt << "\n";
    }
    cout << 0 << "\n";
