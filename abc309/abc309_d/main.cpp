...
int main() {
    ...
    while (!q.empty()) {
        int v = q.front();
        q.pop();
        for (int u = 0; u < n_1 + 1; ++u) {
            if (d.same(v, u) && dist[u] == -1) {
                dist[u] = dist[v] + 1;
                q.push(u);
            }
        }
    ...
    //n_1+n_2からの最短距離を求める
    vector<int> dist2(n_2 + 1, -1);  
    q.push(n_1 + n_2);
    dist2[n_1 + n_2] = 0;
    while (!q.empty()) {
        int v = q.front();
        q.pop();
        for (int u = n_1 + 1; u < n_1 + n_2 + 1; ++u) {
            if (d.same(v, u) && dist2[u] == -1) {
                dist2[u] = dist2[v] + 1;
                q.push(u);
            }
        }
    ...
    int ans = 0;
    // dist と dist2 からそれぞれ最大値を見つける
    int max_dist = *std::max_element(dist.begin(), dist.end());
    int max_dist2 = *std::max_element(dist2.begin(), dist2.end());
    // それらの最大値を足し合わせる
    ans = max_dist + max_dist2;
    std::cout << ans << '\n';
