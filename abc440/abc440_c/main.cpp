...
int main() {
    ...
    while (T--) {
        ...
        // i は 1..N なので (i mod M) を取る（0-basedに合わせるなら (i-1)でもOK。どっちでも「全シフト見る」ので最小値は同じ）
        for (int i = 1; i <= n; i++) {
            bucket[i % M] += c[i - 1];
        }

        // 円環上の長さWの区間和の最小を取るため、2周分に伸ばしてprefix
        vector<long long> pref(2 * M + 1, 0);
        for (int i = 0; i < 2 * M; i++) {
            pref[i + 1] = pref[i] + bucket[i % M];
        }

        long long INF = (1LL << 62);
        long long ans = INF;
        for (int start = 0; start < M; start++) {
            long long sumW = pref[start + W] - pref[start];
            ans = min(ans, sumW);
        }
