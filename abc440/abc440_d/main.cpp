...
int main() {
    ...
    sort(ALL(a));
    vector<int64_t> gap(n, 0);
    REP(i, n-1){
        gap[i+1] = gap[i]+(a[i+1]-a[i]-1);
    }
    REP(i, q){
        int64_t x, y; cin >> x >> y;
        auto itx = lower_bound(ALL(a), x);
        int64_t idxx = distance(a.begin(), itx);
        if(itx == a.end()){
            cout << x+y-1 << "\n";
            continue;
        }
        ...
        y-= cnt;
        auto ity = upper_bound(ALL(gap), gap[idxx] + y - 1);
        int64_t idxy = distance(gap.begin(), ity);

        if (idxy == n) {
            cout << a.back() + (gap[idxx] + y - gap.back()) << "\n";
        } else {
            cout << a[idxy-1] + (y - (gap[idxy-1] - gap[idxx])) << "\n";
        }
    }
    return 0;
}
