...
int main() {
    ...
    int64_t N;
    std::cin >> N;
    std::vector<pair<int64_t, int64_t>> XY(N);
    dsu uf(N);
    REP (i, N) {
        int64_t X, Y;
        std::cin >> X >> Y;
    ...
    REP (i, N) {
        ...
        for(int64_t j=i+1; j < N; ++j){
            if(XY[j].first == X+1 && (XY[j].second == Y+1 || XY[j].second == Y)){
                uf.merge(i, j);
            }
            else if(XY[j].first == X && (XY[j].second == Y-1 || XY[j].second == Y+1)){
                uf.merge(i, j);
            }
            else if(XY[j].first == X-1 && (XY[j].second == Y-1 || XY[j].second == Y)){
                uf.merge(i, j);
            }
        }
    }
