...
int main() {
    ...
    REP(i, h)REP(j, w){
        if(s[i][j] == '#') continue;
        vector<vector<int64_t>> visited(h, vector<int64_t> (w, -1));
        ...
        while(!q.empty()){
            ...
            for(auto [x, y]:directions){
                if(hh+x < 0 || hh+x >= h || ww+y < 0 || ww+y >= w  ||  s[hh+x][ww+y] == '#') continue;
                if(visited[hh+x][ww+y] != -1) continue;
                visited[hh+x][ww+y] = visited[hh][ww] + 1;
                ans = max(ans, visited[hh+x][ww+y]);
                q.push({hh+x, ww+y});
            }
        }
    }
