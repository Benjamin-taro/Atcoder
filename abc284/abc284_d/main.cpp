...
bool isPerfectSquare(int64_t x) {
    int64_t root = static_cast<int64_t>(sqrt(x));
    return root * root == x;
}

void solve(int T, int64_t a) {
    int64_t maximum = static_cast<int64_t>(cbrt(a)); // aの立方根を計算
    for (int64_t i = 2; i <= maximum; ++i) {
        if (a % (i * i) == 0) {
            ...
            int64_t quotient = a / i;
            if (isPerfectSquare(quotient)) {
                int64_t sqrt_quotient = static_cast<int64_t>(sqrt(quotient));
                cout << sqrt_quotient << " " << i << endl;
                return;
            }
        }
    }
