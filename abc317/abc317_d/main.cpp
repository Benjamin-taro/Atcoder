...
int main() {
    ...
    std::cin.tie(nullptr);
    int64_t n; cin >> n;
    vector<pair<int64_t, int64_t>> zyx;
    int64_t taka=0;
    int64_t total = 0;
    REP(i, n){
        int64_t x, y, z; cin >> x >> y >> z;
        total+=z;
        zyx.emplace_back(z, (x>y?0:((x+y)/2)+1-x));
    }
    int64_t INF = 1LL<<60;
    vector<vector<int64_t>> dp(n+1, vector<int64_t>(total+1, INF));
    dp[0][0] = 0;
    REP(i, n){
        REP(j, total+1){
            if(dp[i][j] == INF) continue;
            dp[i+1][j] = min(dp[i+1][j], dp[i][j]);
            dp[i+1][j+zyx[i].first] = min(dp[i+1][j+zyx[i].first], dp[i][j]+zyx[i].second);
        }
    }
    int64_t ans = INF;
