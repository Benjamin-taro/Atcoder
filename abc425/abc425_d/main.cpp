...
vector<pair<int64_t, int64_t>> dir = {
    {1, 0}, {-1, 0}, {0, 1}, {0, -1}
};
...
int main() {
    ...
    while(!q.empty()){
        ...
        while(sz--){
            ...
            for(auto [dirh, drw]: dir){
                int64_t nh = hh + dirh, nw = ww + drw;
                if(nh < 0 || nh >= h || nw < 0 || nw >= w || s[nh][nw] == '#') continue;
                int64_t cnt = 0;
                for(auto [surh, surw]: dir){
                    int64_t sh = nh+surh, sw = nw+surw;
                    if(sh < 0 || sh >= h || sw < 0 || sw >= w) continue;
                    if(s[sh][sw] == '#') cnt++;
                }
                if(cnt == 1) next.push_back({nh, nw});
            }
        }
