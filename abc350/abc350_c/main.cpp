...
auto solve(int64_t N, const std::vector<int64_t> &A) {
    int64_t pointer = 0;
    while(pointer != N){
...
int main() {
    ...
    std::cin >> N;
    std::vector<int> p(N);
    std::vector<int> pos(N + 1); // Positions of elements in the permutation
    ...
    for (int i = 0; i < N; ++i) {
        if (p[i] != i + 1) {
            ...
            int correct_pos = pos[i + 1];

            // Record the swap
            ans.emplace_back(i + 1, correct_pos + 1);

            // Perform the swap in the permutation
            pos[p[i]] = correct_pos;
            pos[i + 1] = i;
            std::swap(p[i], p[correct_pos]);
        }
    }
