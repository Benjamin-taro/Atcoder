...
int64_t solve(int64_t N, int64_t M, const vector<int64_t> &A, const vector<int64_t> &B) {
    dsu uf(N);
    ...
    for (int i = 0; i < N; ++i) {
        if (uf.leader(i) == i) { // グループのリーダーのみを対象とする
            int64_t group_size = uf.size(i);
            int64_t total_possible_friendships = group_size * (group_size - 1) / 2;
            operation_count += total_possible_friendships;
        }
    }
    
    // 既存のエッジ（友達関係）の数を引く
    return operation_count - M;
}
