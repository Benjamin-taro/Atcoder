...
int main() {
    ...
    string s;
    cin >> s;
    vector<int64_t> ca(n+1, 0), cb(n+1, 0);
    for(int64_t i = 0; i < n; ++i) {
        ca[i+1] = ca[i] + (s[i] == 'a' ? 1 : 0);
        cb[i+1] = cb[i] + (s[i] == 'b' ? 1 : 0);
    }
    int64_t ans = 0;
    for(int64_t i = 0; i <= n; ++i) {
        int64_t valid_a, valid_b;
        {
            int64_t left = i, right = n+1;
        ...
        ans += max((int64_t)0, valid_b - valid_a);
    }
    cout << ans << '\n';
    return 0;
}