...
// Aの平方フリー部分を返す
int64_t squarefree(int64_t n) {
    int64_t result = 1;
    for (int64_t p = 2; p * p <= n; p++) {
        int cnt = 0;
        while (n % p == 0) { n /= p; cnt++; }
        if (cnt % 2 == 1) result *= p;  // 奇数回出た素因数のみ残す
    }
    result *= n;  // 残った素因数
    return result;
}
...
int main() {
    ...
    // 0を別処理
    int64_t zeros = count(a.begin(), a.end(), 0LL);
    int64_t ans = zeros * (n - zeros) + zeros * (zeros - 1) / 2;

    // 0以外の平方フリー部分でグループ化
    map<int64_t, int64_t> cnt;
    for (auto x : a) {
        if (x != 0) cnt[squarefree(x)]++;
    }
