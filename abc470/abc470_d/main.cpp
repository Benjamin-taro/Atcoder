...
int main() {
    ...
    int64_t n, q; cin >> n >> q;
    vector<int64_t> p(n);
    vector<int64_t> positonum(n); 
    vector<int64_t> numtoposi(n); 
    REP(i, n){
        cin >> p[i]; p[i]--;
        numtoposi[p[i]] = i;
        positonum[i]=p[i];
    ...
    while(q--){
        ...
        op--;
        if(op){
            swap(numtoposi,positonum);
            sig = !sig;
        }
        else{
            int64_t x, y; cin >> x >> y;
            x--; y--;
            swap(positonum[x],positonum[y]);
            numtoposi[positonum[x]] = x;
            numtoposi[positonum[y]] = y;
        }
    }
    if(sig){
        REP(i, n) cout << positonum[i] << " ";
        cout << "\n";
    }
    else{
        REP(i, n) cout << numtoposi[i] << " ";
        cout << "\n";
    }
