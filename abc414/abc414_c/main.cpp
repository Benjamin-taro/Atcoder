...
int main() {
    ...
    while(true){
        ...
        string pend_odd = s + rev_s.substr(1);
        int64_t p_odd = stoll(pend_odd);
        string pend_even = s + rev_s;
        int64_t p_even = stoll(pend_even);
        if(p_odd > n && p_even > n) break;
        if(p_odd <= n){
            string base_odd = "";
            base_odd = to_base(p_odd, a);
            int l = 0, r = base_odd.size() - 1;
            while(l < r){
                if(base_odd[l] != base_odd[r]) break;
                l++;
                r--;
            }
            if(l >= r) {
        ...
        if(p_even <= n){
            string base_even = "";
            base_even = to_base(p_even, a);
            int l = 0, r = base_even.size() - 1;
            while(l < r){
                if(base_even[l] != base_even[r]) break;
                l++;
                r--;
            }
            if(l >= r) {
