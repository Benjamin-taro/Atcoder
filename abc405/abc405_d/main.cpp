...
int main() {
    ...
    int64_t INF = 1e18;
    std::vector<std::vector<int64_t> > dist(h, vector<int64_t>(w, INF));
    ...
    while(!q.empty()){
        ...
        for(auto [dx, dy] : directions) {
            int64_t nx = x + dx;
            int64_t ny = y + dy;
            if(nx >= 0 && nx < h && ny >= 0 && ny < w) { //Gridの中か否か
                if(s[nx][ny] != '#' && dist[nx][ny] == INF) { //壁ではないかつ未訪問
            ...
            for(auto [dx, dy] : directions) {
                ...
                if(nx >= 0 && nx < h && ny >= 0 && ny < w) { //Gridの中か否か
                    if(dist[nx][ny] == dist[i][j]-1) {
                        if(nx == i-1) ans[i][j] = '^';
                        else if(nx == i+1) ans[i][j] = 'v';
                        else if(ny == j-1) ans[i][j] = '<';
                        else if(ny == j+1) ans[i][j] = '>';
                        break;
                    }
                }
