...
int main() {
    ...
    sort(ALL(posi)); sort(ALL(nega)); reverse(ALL(nega));
    if(posi.size()==0){
        cout << nega[n-1]*(-1) << "\n";
        return 0;
    }
    ...
    int64_t ans=0;
    int64_t posi_cur = 0, nega_cur=0;
    int64_t cur_val = 0;
    bool sig = false;
    while(posi_cur < posi.size() && nega_cur < nega.size()){
        int64_t right = posi[posi_cur]-cur_val;
        int64_t left = cur_val-nega[nega_cur];
        cerr << "cur_val: " << cur_val << ", right: " << right << ", left: " << left << "\n";
        if(left <= right){
            ans += left;
            cur_val = nega[nega_cur];
            nega_cur++;
        }
        else{
            ans += right;
            cur_val = posi[posi_cur];
            posi_cur++;
        }
    }
    if(posi_cur == posi.size()) ans+=cur_val-nega.back();
    if(nega_cur == nega.size()) ans+=posi.back()-cur_val;
    cout << ans <<"\n";
    return 0;
}