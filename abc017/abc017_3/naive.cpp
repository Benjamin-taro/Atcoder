...
int64_t solve(int N, int64_t M, const std::vector<int64_t> &l, const std::vector<int64_t> &r, const std::vector<int64_t> &s) {
    // TODO: edit here
}
...
int main() {
    ...
    int64_t M;
    std::cin >> N;
    std::vector<int64_t> l(N), r(N), s(N);
    std::cin >> M;
    REP (i, N) {
        std::cin >> l[i] >> r[i] >> s[i];
    }
    auto ans = solve(N, M, l, r, s);
    std::cout << ans << '\n';
    return 0;
}
