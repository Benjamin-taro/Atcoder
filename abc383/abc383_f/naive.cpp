...
int64_t solve(int N, int64_t X, int64_t K, const std::vector<int64_t> &P, const std::vector<int64_t> &U, const std::vector<int64_t> &C) {
    // TODO: edit here
}
...
int main() {
    ...
    int64_t X, K;
    std::cin >> N;
    std::vector<int64_t> P(N), U(N), C(N);
    std::cin >> X >> K;
    REP (i, N) {
        std::cin >> P[i] >> U[i] >> C[i];
    }
    auto ans = solve(N, X, K, P, U, C);
    std::cout << ans << '\n';
    return 0;
}
