...
int main() {
    ...
    std::cin.tie(nullptr);
    int64_t n, m; cin >> n >> m;
    vector<vector<pair<int64_t, int64_t>>> world(n);
    REP(i, m){
        int64_t p, y; cin >> p >> y;
        p--;
        world[p].push_back({y, i});
    }
    vector<string> ans(m);
    REP(i, n){
        vector<pair<int64_t, int64_t>> cities = world[i];
        if(cities.empty()) continue;
        sort(ALL(cities));
        string country_code = make_code(i+1);
        int64_t city_cnt = 1;
        for(auto[yy, idx]:cities){
            ans[idx] = country_code+make_code(city_cnt);
            city_cnt++;
        }
    }
    for(auto code:ans) cout << code << "\n";
    return 0;
}
