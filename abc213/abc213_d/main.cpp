...
void dfs(int crr, int pre) {
    ans.push_back(crr);  // 現在のノードを結果に追加
    for (int nxt : G[crr]) {
        if (nxt != pre) {  // 前のノードでなければ
            dfs(nxt, crr);  // 次のノードを探索
            ans.push_back(crr);  // 元のノードに戻る
        }
    }
}
...
int main() {
    ...
    for (int i = 1; i <= N; i++) {
        sort(G[i].begin(), G[i].end());  // 隣接リストをソートして、順番を一定にする
    }

    dfs(1, -1);  // 1から探索を開始、親ノードは存在しないので-1
