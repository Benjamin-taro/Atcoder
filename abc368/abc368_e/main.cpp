...
auto solve(auto N, auto M, const std::vector<auto> &X, const std::vector<auto> &A, const std::vector<auto> &B, const std::vector<auto> &S, const std::vector<auto> &T) {
    // TODO: edit here
}
...
int main() {
    ...
    std::vector<auto> X(0);
    std::cin >> N >> M;
    std::vector<auto> A(M), B(M), S(M), T(M);
    std::cin >> X[0];
    REP (i, M) {
        std::cin >> A[i] >> B[i] >> S[i] >> T[i];
    }
    auto ans = solve(N, M, X, A, B, S, T);
