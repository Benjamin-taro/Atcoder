...
bool important[MAX_N + 1] = {false};
bool visited[MAX_N + 1] = {false};
int vertexCount = 0;

bool dfs(int node, int parent) {
    visited[node] = true;
    bool hasImportant = important[node];

    for (int neighbor : tree[node]) {
        if (neighbor == parent) continue; // 親ノードへ戻るのを防ぐ
...
int main() {
    ...
    // 重要な頂点を読み込む
    vector<int> importantVertices(K);
    for (int i = 0; i < K; i++) {
        cin >> importantVertices[i];
        important[importantVertices[i]] = true;
    }

    // 重要な頂点の1つからDFSを開始
    dfs(importantVertices[0], -1);
